...
function convertToRomanNumerals(num) {
  ...
  const remainderDecimal = num % 10;
  if (remainderDecimal < 4) {
    for (let i = 0; i < remainderDecimal; i += 1) {
      result += 'I';
    }
  } else if (remainderDecimal === 4) {
    result += 'IV';
  } else if (remainderDecimal > 4 && remainderDecimal < 9) {
    result += 'V';
    const unit = remainderDecimal % 5;
...
function isPalindrome(str) {
  ...
  const end = str.length % 2 === 0 ? length : length - 1;
  let head = '';
  let tail = '';
  for (let i = 0; i < length; i += 1) {
    head += str[i];
  }
  for (let j = str.length - 1; j >= end; j -= 1) {
    tail += str[j];
  }
  return head === tail;
}
...
function getSpiralMatrix(size) {
  ...
  const getSpiral = (startIndex, newSize, array) => {
    const arr = array;
    const sizeIndex = newSize - 1;
    for (let i = startIndex; i < newSize; i += 1) {
      arr[startIndex][i] = current;
      if (i !== newSize - 1) {
        current += 1;
      }
    }
    for (let i = startIndex; i < newSize; i += 1) {
      arr[i][sizeIndex] = current;
      if (i !== newSize - 1) {
        current += 1;
      }
    }
    for (let i = sizeIndex; i >= startIndex; i -= 1) {
      arr[sizeIndex][i] = current;
      if (i !== startIndex) {
        current += 1;
      }
    }
    for (let i = sizeIndex; i > startIndex; i -= 1) {
      arr[i][startIndex] = current;
      current += 1;
    }
    return arr;
  };
  ...
  while (current < maxNumber) {
    getSpiral(start, size - start, newArr);
    start += 1;
  }
  getSpiral(start, size - start, newArr);
  return newArr;
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix.length; j += 1) {
      arr[j][i] = matrix[matrix.length - 1 - i][j];
    }
  }
  const newArr = matrix;
...
function sortByAsc(arr) {
  ...
  const temporary = [...sortByAsc(leftArr), pivot, ...sortByAsc(rightArr)];
...
function shuffleChar(str, iterations) {
  ...
  while (iter <= iterations) {
    ...
    for (let i = 0; i < newStr.length; i += 1) {
      if (i % 2 === 0) {
        strEvenIndex += newStr[i];
      } else {
        strOddIndex += newStr[i];
      }
    }

    newStr = strEvenIndex + strOddIndex;
    if (newStr === str) {
